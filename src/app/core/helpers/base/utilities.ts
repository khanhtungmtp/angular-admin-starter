...
export class Utilities {
  ...
  public static getHttpResponseMessage(
    ...
    if (!httpMessage) {
      for (const msgKey of preferredMessageKeys) {
        ...
        if (httpMessage?.trim() !== '') return httpMessage;
      }
    }
    ...
    if (!httpMessage) {
      ...
      if (responseMessages.length) httpMessage = responseMessages.join('\n');
    }
  ...
  public static findHttpResponseMessage(
    searchString: string,
    data: HttpResponseBase,
    searchOptions?: HttpMessageSearchOptions
  ): string | null {
    ...
    for (const httpMsg of httpMessages) {
      ...
      if (searchOptions.searchInCaption && searchOptions.searchInMessage) messageToSearch = httpMsg;
      else if (searchOptions.searchInCaption) messageToSearch = captionAndMessage.caption;
      else if (searchOptions.searchInMessage) messageToSearch = captionAndMessage.message ?? '';
  ...
  public static stringify(value: unknown, depth = 3): string {
    const worker = (value: unknown, depth: number, padding = ''): string => {
      ...
      if (typeof value === 'object') {
        ...
        if (result !== objectobject) return result;
        ...
        for (const key in value) {
          if (Object.prototype.hasOwnProperty.call(value, key)) {
            ...
            if (typeof keyEntry !== 'function') {
              const keyValue =
                depth > 0 ? worker(keyEntry, depth - 1, padding + ' ') : String(keyEntry);
