...
import { EndpointBaseService } from '../base/endpoint-base.service';
import { ConfigurationService } from '../base/configuration.service';

@Injectable({
  providedIn: 'root',
})
export class AccountEndpointService extends EndpointBaseService {
  get usersUrl() {
    return this.configurations.apiUrl + '/account/users';
  }
  get userByUserNameUrl() {
    return this.configurations.apiUrl + '/account/users/username';
  }
  get currentUserUrl() {
    return this.configurations.apiUrl + '/account/users/me';
  }
  get currentUserPreferencesUrl() {
    return this.configurations.apiUrl + '/account/users/me/preferences';
  }
  get unblockUserUrl() {
    return this.configurations.apiUrl + '/account/users/unblock';
  }
  get rolesUrl() {
    return this.configurations.apiUrl + '/account/roles';
  }
  get roleByRoleNameUrl() {
    return this.configurations.apiUrl + '/account/roles/name';
  }
  get permissionsUrl() {
    return this.configurations.apiUrl + '/account/permissions';
  }
  ...
  getPatchUpdateUserEndpoint<T>(
    ...
    if (path) {
      ...
      patchDocument = from
        ? [{ op: opOrUserId, from, path }]
        : [{ op: opOrUserId, path, value: valueOrPatch }];
    } else {
    ...
    return this.http.patch<T>(endpointUrl, JSON.stringify(patchDocument), this.requestHeaders).pipe(
      catchError(error => {
        return this.handleError(error, () =>
          this.getPatchUpdateUserEndpoint<T>(
            valueOrPatch,
            opOrUserId as string,
            path as string,
            from,
            userId
          )
  ...
  getUpdateUserPreferencesEndpoint<T>(configuration: string | null): Observable<T> {
    return this.http
      .put<T>(this.currentUserPreferencesUrl, JSON.stringify(configuration), this.requestHeaders)
      .pipe(
        catchError(error => {
          return this.handleError(error, () =>
            this.getUpdateUserPreferencesEndpoint<T>(configuration)
          );
        })
      );
  }
