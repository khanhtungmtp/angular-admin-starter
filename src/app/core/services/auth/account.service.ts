...
import { Permission, PermissionValues } from '@core/models/system/users/permission.model';
import { Role } from '@core/models/system/users/role.model';
import { User } from '@core/models/system/users/user.model';
import { Observable, Subject, forkJoin, mergeMap, tap } from 'rxjs';
import { AuthService } from './auth.service';
import { AccountEndpointService } from './account-endpoint.service';
import { UserEdit } from '@core/models/system/users/user-edit.model';
...
export class AccountService {
  ...
  constructor(private authService: AuthService, private accountEndpoint: AccountEndpointService) {}
  ...
  deleteUser(userOrUserId: string | User): Observable<User> {
    if (typeof userOrUserId === 'string') {
      return this.accountEndpoint
        ...
        .pipe<User>(tap(user => this.onRolesUserCountChanged(user.roles)));
    } else {
      if (userOrUserId.id) {
  ...
  updateRole(role: Role) {
    if (role.id) {
      return this.accountEndpoint
        ...
        .pipe(tap(() => this.onRolesChanged([role], AccountService.roleModifiedOperation)));
    } else {
  ...
  newRole(role: Role) {
    return this.accountEndpoint
      ...
      .pipe<Role>(tap(() => this.onRolesChanged([role], AccountService.roleAddedOperation)));
  }
  ...
  deleteRole(roleOrRoleId: string | Role): Observable<Role> {
    if (typeof roleOrRoleId === 'string') {
      return this.accountEndpoint
        ...
        .pipe<Role>(tap(data => this.onRolesChanged([data], AccountService.roleDeletedOperation)));
    } else {
      if (roleOrRoleId.id) {
