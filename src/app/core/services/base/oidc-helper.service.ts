...
import { LoginResponse } from '@core/models/system/users/login-response.model';
import { LocalStoreManagerService } from './localstore-manager.service';
...
export class OidcHelperService {
  ...
  get accessToken(): string | null {
    return this.localStoreManagerService.getData(DBkeys.ACCESS_TOKEN);
  }

  get accessTokenExpiryDate(): Date | null {
    return this.localStoreManagerService.getDataObject<Date>(DBkeys.TOKEN_EXPIRES_IN, true);
  }

  get refreshToken(): string | null {
    return this.localStoreManagerService.getData(DBkeys.REFRESH_TOKEN);
  }
